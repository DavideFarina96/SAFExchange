...
router.get('/:action_id', async function (req, res) {
	// Return the planned action with same _id as params.action_id
	try {
...
router.delete('/:action_id', async function (req, res) {
	// Delete planned action (set its state to CANCELED)
	try {
...
router.post('/checkTriggers', async function (req, res) {
	// This function is called by /price when a new value for BTC/ETH is avaiable
	// Check if there is any planned action to be triggered
	// If there is, insert new transaction, update user balance and set state to COMPLETED
