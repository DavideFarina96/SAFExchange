...
router.post('/buy/:plannedaction_id', function (req, res) { 
	...
	{	
		...
		var _action =  req.body.action; // buy or sell 
		...
		sdtwsdb.then(function(result) {
			...
			{	
				...
				//var sdtwspa = sendDataToWS('localhost', 8080, path, 'DELETE',  _header, {}); 
				//sdtwspa.then(function(result) {
					// step 4: save the transaction in the db (call /database/.... )
					var path = "/database/transaction/";	
					var sdtwsdb = sendDataToWS('localhost', 8080, path, 'POST',  _header, dataObj);
					sdtwsdb.then(function(result) {

						// set 5: update user balance by calling /user/... with PUT method (simulate the action "buy")
						var path = "/user/balance/buy/";	
						var sdtwsuser = sendDataToWS('localhost', 8080, path, 'PUT',  _header, dataObj);
